...
export class PaymentsService {
  constructor(
    ...
    private readonly expiration: PaymentExpirationService,
  ) {}
  ...
  async checkPaymentStatus(paymentId: PaymentEntity['id'], customerId: string) {
    return this.dataSource.transaction(async (manager) => {
      ...
      let payment = await paymentsRepo.findOneBy({ id: paymentId });
      ...
      const order = await manager
        ...
        .findOneBy({ paymentId: payment.id, customerId });
      if (!order) throw new BadRequestException('Payment does not exist');
      await this.expiration.reconcileCheckout(manager, customerId, order.id);
      payment = (await paymentsRepo.findOneBy({ id: paymentId })) || payment;
      ...
      return {
        ...
        paidAmount:
          payment.paidAmount == null ? null : Number(payment.paidAmount),
