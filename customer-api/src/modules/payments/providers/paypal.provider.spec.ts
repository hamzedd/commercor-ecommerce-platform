...
describe('PayPalPaymentProvider', () => {
  ...
  it('obtains and caches OAuth then creates a server-priced order idempotently', async () => {
    const fetchMock = jest
      ...
      .mockResolvedValueOnce(
        response({
          id: 'PAYPAL-ORDER',
          links: [{ rel: 'approve', href: 'https://sandbox.paypal.test' }],
        }),
      )
    ...
    expect(
      (create[1]?.headers as Record<string, string>)['PayPal-Request-Id'],
    ...
    expect(JSON.parse(String(create[1]?.body))).toEqual(
      expect.objectContaining({
        ...
        purchase_units: [
          expect.objectContaining({
            custom_id: 'payment',
            amount: { value: '12.34', currency_code: 'EUR' },
          }),
        ],
      }),
    );
  ...
  it('normalizes only a completed capture', async () => {
    ...
    await expect(
      new PayPalPaymentProvider().captureOrder('ORDER', 'key'),
    ).resolves.toEqual({
  ...
  it('rejects a non-completed capture', async () => {
    jest
      ...
      .mockResolvedValueOnce(
        response({ id: 'ORDER', status: 'APPROVED', purchase_units: [] }),
      );
    await expect(
      new PayPalPaymentProvider().captureOrder('ORDER', 'key'),
  ...
  it('rejects an invalid webhook and accepts a verified one', async () => {
    ...
    const headers = {
      ...
      'paypal-transmission-time': '2026-08-12T00:00:00Z',
    };
    jest
      .spyOn(global, 'fetch')
      .mockResolvedValueOnce(
    ...
    expect(provider.parseVerifiedEvent(verified)).toEqual(
      expect.objectContaining({
        paymentId: 'payment',
        externalTransactionId: 'CAPTURE',
      }),
  ...
  it('normalizes only confirmed provider refunds', async () => {
    ...
    await expect(
      new PayPalPaymentProvider().refundPayment({
        paymentId: 'payment',
        externalTransactionId: 'CAPTURE',
        amount: 5,
        currencyCode: 'USD',
