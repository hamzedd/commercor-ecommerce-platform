...
export class PayPalPaymentProvider implements PaymentProvider {
  ...
  private normalizeCompletedOrder(order: PayPalOrder): NormalizedPaymentEvent {
    ...
    if (!unit?.custom_id)
      throw new BadRequestException('PayPal payment reference is missing');
    return {
  ...
  async verifyWebhook(request: VerifyWebhookRequest): Promise<VerifiedWebhook> {
    ...
    if (!webhookId)
      throw new UnauthorizedException('PayPal webhook is not configured');
  ...
  parseVerifiedEvent(webhook: VerifiedWebhook): NormalizedPaymentEvent {
    if (webhook.provider !== this.name)
      throw new UnauthorizedException('Webhook provider mismatch');
    const event = webhook.payload as PayPalWebhookEvent;
    const resource = event.resource;
    const paymentId = String(resource.custom_id || resource.invoice_id || '');
    const amount =
      resource.amount || resource.seller_payable_breakdown?.gross_amount;
    ...
    if (
      ...
      throw new BadRequestException(
        'PayPal webhook is missing payment details',
      );
    }
    return {
      type,
      ...
      relatedTransactionId:
        type === NormalizedPaymentEventType.REFUND_COMPLETED
          ? String(
              resource.links?.find((link: any) => link.rel === 'up')?.href ||
                '',
            )
              .split('/')
              .pop()
  ...
  async refundPayment(
    ...
    return {
      ...
      pending: refund.status !== 'COMPLETED',
    };
  }
}
