...
export class SafeExceptionFilter implements ExceptionFilter {
  private readonly logger = new Logger(SafeExceptionFilter.name);
    ...
    if (status < 500 && typeof body === 'object') return response.status(status).json(body);
    if (status < 500) return response.status(status).json({ statusCode: status, message: body });
