...
export function rateLimiter(req: Request, res: Response, next: NextFunction) {
  ...
  const loginPath = req.path.endsWith('/auth/login') && req.method === 'POST';
  const limit = loginPath
    ? positiveInt('AUTH_RATE_LIMIT_MAX', 10)
    : positiveInt('RATE_LIMIT_MAX', 300);
  const key = `${req.ip || req.socket.remoteAddress || 'unknown'}:${loginPath ? 'login' : 'api'}`;
  const now = Date.now();
